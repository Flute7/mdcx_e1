...
class MyMAinWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.show_net_info(
            '\n💡 Explanation: \n '
            'Any Proxy:           javbus, jav321, javlibrary, mgstage, mywife, giga, freejavbt, mdtv, madouqu,\n '
            '                     7mmtv, faleno, dahlia, prestige, theporndb, cnmdb, fantastica, kin8\n '
            'Non-Japanese Proxy:  javdb, airav-cc, avsex（Japanese proxy will report an error）\n '
            'Japanese Proxy:      seesaawiki\n '
            'No Proxy:            avsex, hdouban, iqqtv, airav-wiki, love6, lulubar, fc2, fc2club, fc2hub\n\n'
    ...
    def _show_stop_info(self):
        ...
        try:
            ...
            signal.show_log_text(' ⏱ Runtime:'.ljust(13) + '%ss' % used_time)
            signal.show_log_text(' 🍕 Average Time:'.ljust(14) + '%ss' % average_time)
            signal.show_log_text("================================================================================")
...
When you view and download the source code or binary program of this project, you accept the following terms:
    ...
    def reset_buttons_status(self):
        ...
        if len(Flags.failed_list):
            self.Ui.pushButton_scraper_failed_list.setText(f'Re-scrape Failed ({len(Flags.failed_list)})')
        else:
